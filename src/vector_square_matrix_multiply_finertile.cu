...
#define MATRIX_WIDTH     14u  /* Width of the square matrix, 14x14 */
...
__global__ void MatrixMultiply(float *A, float *B, float *C)
{
    ...
    if ((row < MATRIX_WIDTH) && (col < MATRIX_WIDTH))
    {
        ...
        {
            Pvalue += A[row * MATRIX_WIDTH + i] * B[i * MATRIX_WIDTH + col];
        }
        C[row * MATRIX_WIDTH + col] = Pvalue;
    }
}
...
void cpuMatrixMultiply(float *A, float *B, float *C)
{
    for (int i = 0; i < MATRIX_WIDTH; i++)
    {
        for (int j = 0; j < MATRIX_WIDTH; j++)
        {
            ...
            {
                sum += A[i * MATRIX_WIDTH + k] * B[k * MATRIX_WIDTH + j];
            }
            C[i * MATRIX_WIDTH + j] = sum;
        }
    }
}
...
void printMatrix(float *matrix)
{
    for (int i = 0; i < MATRIX_WIDTH; i++)
    {
        for (int j = 0; j < MATRIX_WIDTH; j++)
        {
            printf("%f ", matrix[i * MATRIX_WIDTH + j]);
        }
        printf("\n");
    }
}
...
int main(void)
{
    ...
    dim3 dimGrid(ceil(1.0 * (MATRIX_WIDTH / TILE_WIDTH)), ceil(1.0 * (MATRIX_WIDTH / TILE_WIDTH)));
